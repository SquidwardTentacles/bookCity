        ...
        <div class="form">
          ...
          <ul class="form-group">
            <li class="info-name">
              <input type="text"
                     v-focus
                     :class="{errli:form.nameErr}"
                     name="name"
                     placeholder="Name"
                     v-model="applyInfo.name">
              <p class="errP"
                 v-if="form.nameErr">{{form.nameMsg}}</p>
            </li>
            <li class="info-email">
              <input type="text"
                     :class="{errli:form.emailErr}"
                     name="email"
                     placeholder="Email Address"
                     v-model="applyInfo.email">
              <p class="errP"
                 v-if="form.emailErr">{{form.emailMsg}}</p>
            </li>
            <li class="info-mobile">
              <input type="number"
                     :class="{errli:form.mobileErr}"
                     name="mobile"
                     placeholder="Mobile Number"
                     v-model="applyInfo.mobile">
              <p class="errP"
                 v-if="form.mobileErr">{{form.mobileMsg}}</p>
            </li>
            <li class="info-location">
              <input type="text"
                     :class="{errli:form.locationErr}"
                     name="location"
                     placeholder="Current Location"
                     v-model="applyInfo.location">
              <p class="errP"
                 v-if="form.locationErr">{{form.locationMsg}}</p>
            </li>
            <li class="reason-li"
                :class="{'info-reason':applyInfo.joinReason.split('').length >= 300}">
              <textarea :class="{errli:form.joinReasonErr}"
                        name="joinReason"
                        v-model="applyInfo.joinReason"
                        rows="5"
                        cols="20"
                        placeholder="Why do you want to join HappyEasy Go?"></textarea>
              <span>{{wordNumber}}</span>
              <p class="errP"
                 v-if="form.joinReasonErr">{{form.joinReasonMsg}}</p>
            </li>
            <li class="info-link">
              <input type="text"
                     :class="{errli:form.linkedinUrlErr}"
                     name="linkedinUrl"
                     placeholder="LinkedIn URL (Optional)"
                     v-model="applyInfo.linkedinUrl">
              <p class="errP"
                 v-if="form.linkedinUrlErr">{{form.linkedinUrlMsg}}</p>
            </li>
            <li class="info-file">
              <span class="file-span"
                    v-if="fileObj != null"
                    v-show="isSelectFile">{{fileObj.name}}</span>
            ...
            <li class="info-btns flex space-between">
              <button @click="infoJustify"
                      :class="{active:true}">Submit</button>
            </li>
          </ul>
        </div>
      </form>
...
export default {
  ...
  data () {
    return {
      ...
      form: { // 对应input错误信息的设置
        ...
        mobileErr: false,
        locationMsg: 'Please enter your current location',
        locationErr: false,
        joinReasonMsg: 'Please write down your reason',
        joinReasonErr: false,
        linkedinUrlMsg: '',
        linkedinUrlErr: false,
  ...
  computed: {
    wordNumber () { // 动态计算textarea的字数
      const number = this.applyInfo.joinReason.split('').length
      return (300 - number)
    }
  },
  watch: {
  ...
  methods: {
    selectFile () {
      ...
      if (this.fileObj) { // 原来的代码,稍作调整
        ...
        const idx = this.fileObj.name.lastIndexOf('.')
        if (idx != -1) {
    ...
    fileUpload () {
      ...
      // 这里上传文件并且需要传更多参数的时候
      const fileForm = document.getElementById('fileForm')// 获取整个form表单数据,记住input都需要name属性
      const url = DomainManager.saveCareer()// 上传服务器接口
